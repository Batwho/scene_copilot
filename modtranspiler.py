...
#Path(bpy.path.basename(bpy.context.blend_data.filepath)).stem + ".py")
...
if mode == "print":
    ...
    filename = f'{Path(bpy.path.basename(bpy.context.blend_data.filepath)).stem}'
    while exists(filename + '.py'):
        filename = filename + '1'
