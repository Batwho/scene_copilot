...
def shader_material_013(nw: NodeWrangler):
    # Code generated using version 2.6.5 of the node_transpiler

    principled_bsdf = nw.new_node(Nodes.PrincipledBSDF,
        input_kwargs={'Base Color': (0.8000, 0.3468, 0.4010, 1.0000), 'Metallic': 1.0000, 'Roughness': 0.1636})
...
def geometry_nodes(nw: NodeWrangler):
    ...
    add = nw.new_node(Nodes.Math, input_kwargs={0: multiply_1, 1: group_input.outputs["Inner spread"]})
    ...
    cosine = nw.new_node(Nodes.Math, input_kwargs={0: reroute_1}, attrs={'operation': 'COSINE'})
    ...
    node_utils.assign_curve(float_curve_2.mapping.curves[0], [(0.0000, 0.0010), (0.1058, 0.6375), (0.3663, 0.7966), (0.8747, 0.8188), (0.9667, 0.1038), (1.0000, 0.0188)])
    ...
    resample_curve_1 = nw.new_node(Nodes.ResampleCurve, input_kwargs={'Curve': set_curve_radius_1, 'Count': 55})
    
    quadrilateral = nw.new_node('GeometryNodeCurvePrimitiveQuadrilateral', input_kwargs={'Width': 1.8100, 'Height': 0.1700})
    
    subdivide_curve = nw.new_node(Nodes.SubdivideCurve, input_kwargs={'Curve': quadrilateral, 'Cuts': 2})
    ...
    join_geometry_4 = nw.new_node(Nodes.JoinGeometry, input_kwargs={'Geometry': curve_to_mesh_1})
    
    set_material = nw.new_node(Nodes.SetMaterial,
        input_kwargs={'Geometry': join_geometry_4, 'Material': surface.shaderfunc_to_material(shader_material_013)})
    ...
    instance_on_points = nw.new_node(Nodes.InstanceOnPoints,
        input_kwargs={'Points': set_position_1, 'Instance': set_material, 'Rotation': add_1.outputs["Vector"], 'Scale': multiply_10})
...
def apply(obj, selection=None, **kwargs):
    ...
    surface.add_material(obj, shader_material_013, selection=selection)
